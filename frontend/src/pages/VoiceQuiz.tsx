...
export default function VoiceQuiz() {
  ...
  useEffect(() => {
    setUserAnswers(new Array(voiceQuizData.length).fill(null));
  }, []);

  // Helper to parse bold text from markdown-style **text** to HTML
  const parseQuestionText = (text: string) => {
    return text.replace(/\*\*(.*?)\*\*/g, '<span class="font-bold text-blue-700">$1</span>');
  };
  ...
  // Handle Navigation
  const handleNext = () => {
    if (currentQuestionIndex < voiceQuizData.length - 1) {
  ...
  if (isFinished) {
    const score = userAnswers.reduce((acc, ans, index) => {
      return (ans === voiceQuizData[index].correctAnswer) ? (acc || 0) + 1 : (acc || 0);
    }, 0) || 0;

    const percentage = Math.round((score / voiceQuizData.length) * 100);
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-10 px-4">
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          >
            <div className="w-24 h-24 bg-blue-100 rounded-full flex items-center justify-center mx-auto mb-6 text-5xl shadow-sm">
              🏆
            </div>
            
            <h2 className="text-3xl font-black text-slate-800 mb-2">Quiz Completed!</h2>
            <p className="text-slate-500 mb-8 font-medium">ทดสอบ Active & Passive Voice เรียบร้อย มาดูผลคะแนนกัน!</p>
            
            <div className="flex justify-center items-end gap-2 mb-8">
              <span className="text-6xl font-black text-blue-600">{score}</span>
              <span className="text-2xl font-bold text-slate-400 mb-2">/ {voiceQuizData.length}</span>
            </div>
          ...
          <div className="space-y-6">
            ...
            {voiceQuizData.map((question, index) => {
  ...
  // --- Quiz Screen ---
  const questionData = voiceQuizData[currentQuestionIndex];
  const progress = ((currentQuestionIndex + 1) / voiceQuizData.length) * 100;
  const isLastQuestion = currentQuestionIndex === voiceQuizData.length - 1;
    ...
    <div className="min-h-screen bg-slate-50 py-8 px-4 flex flex-col items-center">
      <div className="w-full max-w-3xl flex items-center justify-between mb-6">
        ...
        <div className="px-4 py-1.5 bg-blue-100 text-blue-700 rounded-full text-sm font-bold">
          ข้อที่ {currentQuestionIndex + 1} / {voiceQuizData.length}
        </div>
      </div>
